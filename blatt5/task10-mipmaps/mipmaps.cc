...
loadMipmapTexture(std::vector<std::string> const &filenames, unsigned int *id) {
    ...
    for(std::string fileName: filenames) {
        all_data.push_back(stbi_load(fileName.c_str(), &width, &height, &nrChannels, 0));
    }
    ...
    int height_weight = 512;
    for(int i = 0; i < all_data.size(); i++) {
        if (all_data.at(i)) {
            glTexImage2D(GL_TEXTURE_2D, i, GL_RGB, height_weight, height_weight, 0, GL_RGB, GL_UNSIGNED_BYTE, all_data.at(i));
        } else {
            std::cout << "Failed to load texture" << std::endl;
        }
        stbi_image_free(all_data.at(i));
        height_weight/=2;
    }
